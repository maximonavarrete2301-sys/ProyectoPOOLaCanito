package controlador;//Victor Diaz, Maximo Navarrete y Rodrigo Henriquez
...
public class SistemaVentaPasajes {
    ...
    private ArrayList<modelo.Viaje> viajes;
    ...
    public boolean createBus(String patente, String marca, String modelo, int nroAsientos) {
        ...
        modelo.Bus nuevo = new modelo.Bus(patente, nroAsientos);
    ...
    public boolean createViaje(LocalDate fecha, LocalTime hora, int precio, String patBus) {
        ...
        if (busEncontrado != null && !viajeRepetido) {
            modelo.Viaje nuevo = new modelo.Viaje(fecha, hora, precio, busEncontrado);
            viajes.add(nuevo);
            return true;
        }
        return false;
    ...
    public boolean iniciaVenta(String idDoc, TipoDocumento tipo, LocalDate fechaVenta, IdPersona idCliente) {
        ...
        if (clienteEncontrado != null && !ventaRepetida) {
            utilidades.Venta nueva = new utilidades.Venta(idDoc, tipo, fechaVenta, clienteEncontrado);
            ventas.add(nueva);
            return true;
        }
        return false;
    ...
    public String[][] getHorariosDisponibles(LocalDate fechaViaje) {
        ArrayList<modelo.Viaje> filtrados = new ArrayList<>();
        for (int i = 0; i < viajes.size(); i++) {
        ...
        for (int i = 0; i < filtrados.size(); i++) {
            modelo.Viaje v = filtrados.get(i);
    ...
    public String[] listAsientosDeViaje(LocalDate fecha, LocalTime hora, String patBus) {
        modelo.Viaje viajeEncontrado = null;
        for (int i = 0; i < viajes.size(); i++) {
            if (viajes.get(i).getFecha().equals(fecha) &&
                    viajes.get(i).getHora().equals(hora) &&
                    viajes.get(i).getBus().getPatente().equals(patBus)) {
    ...
    public boolean vendePasaje(String idDoc, LocalDate fecha, LocalTime hora, String patBus, int asiento, IdPersona idPasajero) {
        ...
        utilidades.Venta ventaEncontrada = null;
        for (int i = 0; i < ventas.size(); i++) {
    ...
    public String[][] listVentas() {
        ...
        for (int i = 0; i < ventas.size(); i++) {
            utilidades.Venta v = ventas.get(i);
